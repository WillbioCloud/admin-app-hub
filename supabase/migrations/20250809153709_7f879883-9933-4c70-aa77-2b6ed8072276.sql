...
begin
  select exists (
    select 1 from public.user_completed_missions
  ...
  set
    xp = xp + (select xp_reward from public.missions where id = p_mission_id),
    coins = coins + (select coin_reward from public.missions where id = p_mission_id)
  where id = auth.uid();
